...
| `--preset <name>` | Wire the MCP server to a tool preset: `navigation` (the lean default), `minimal`, `memory`, `verify`, `federation`, `coordination`, or `full`. Omit it for the lean navigation surface; pass `--preset full` to wire all 68 tools (the prior default). |
...
> winner — not all 68. This is per-session token savings with no capability loss: every tool stays one
...
> install with **`--preset full`** (all 68) or **`--preset minimal`** (the governance core) to wire them.
