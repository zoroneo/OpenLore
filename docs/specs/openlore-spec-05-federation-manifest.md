...
- [x] Tests (`src/cli/manifest/manifest.test.ts`, co-located — `test/` is gitignored): emitted manifest validates against the real schema, determinism, entry-point default vs `--include-private`, `--max-symbols` truncation, re-export resolution, stats/languages, routes/packages, validator negatives. 17 tests.
