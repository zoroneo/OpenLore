...
If you only need one server entry and want every tool searchable, use `alwaysLoad: false` (the default) with `openlore mcp --preset full` — all 72 tools are deferred and searchable via Tool Search. A bare `openlore mcp` instead gives the 13-tool `substrate` default surface (the navigation core plus governance reads; use `--preset navigation` for the lean 10-tool core).
